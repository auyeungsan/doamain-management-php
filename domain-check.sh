...
check_domain_status()
{
    ...
    then
        ${WHOIS} -h "whois.twnic.net.tw" "${1}" > ${WHOIS_TMP}
    elif [ "${TLDTYPE}"  == "cc" ];
    then
        ${WHOIS} -h "whois.nic.cc" "${1}" > ${WHOIS_TMP}
    elif [ "${TLDTYPE}"  == "online" ];
    then
        ${WHOIS} -h "whois.nic.online" "${1}" > ${WHOIS_TMP}
    elif [ "${TLDTYPE}"  == "xyz" ];
    then
        ${WHOIS} -h "whois.nic.xyz" "${1}" > ${WHOIS_TMP}
    elif [ "${TLDTYPE}"  == "mobi" ];
    then
        ${WHOIS} -h "whois.dotmobiregistry.net" "${1}" > ${WHOIS_TMP}
    elif [ "${TLDTYPE}"  == "site" ];
    then
        ${WHOIS} -h "whois.centralnic.com" "${1}" > ${WHOIS_TMP}
    elif [ "${TLDTYPE}"  == "studio" ];
    then
        ${WHOIS} -h "whois.rightside.co" "${1}" > ${WHOIS_TMP}
    elif [ "${TLDTYPE}"  == "today" ];
    then
    ...
    then
        count=0
        while [ $count -le 3 ];do
                whoisResult=$(${WHOIS} -h "whois.cnnic.net.cn" "${1}" > ${WHOIS_TMP})
